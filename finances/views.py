...
def financial_dashboard(request):
    ...
    total_events = Event.objects.filter(date__month=current_month,
                                        date__year=current_year).count()

    # Dados financeiros do mês
    monthly_income = Transaction.objects.filter(
        type='income', date__month=current_month,
        date__year=current_year).aggregate(total=Sum('amount'))['total'] or 0

    monthly_expense = Transaction.objects.filter(
        type='expense', date__month=current_month,
        date__year=current_year).aggregate(total=Sum('amount'))['total'] or 0
    ...
    for i in range(5, -1, -1):
        ...
        income = Transaction.objects.filter(
            type='income', date__month=month.month,
            date__year=month.year).aggregate(total=Sum('amount'))['total'] or 0
        expense = Transaction.objects.filter(
            type='expense', date__month=month.month,
            date__year=month.year).aggregate(total=Sum('amount'))['total'] or 0
